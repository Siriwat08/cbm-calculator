...
// Inline version of the algorithm with debug output
const truck = {
  id: 'pickup',
...
console.log('Effective truck:', effectiveW, 'x', effectiveL, 'x', effectiveH);

// Pre-place obstacles
const obstacleBoxes = (truck.obstacles || []).map((obs, idx) => ({
  x: obs.x, y: obs.y, z: obs.z,
  width: obs.width, length: obs.length, height: obs.height,
  cargoIndex: -1, itemIndex: -1,
}));
...
// Check: are obstacles within effective bounds?
obstacleBoxes.forEach((o, i) => {
  const inBounds = o.x + o.width <= effectiveW && o.y + o.length <= effectiveL && o.z + o.height <= effectiveH;
  console.log(`  Obstacle ${i} in effective bounds: ${inBounds}`);
  if (!inBounds) {
    console.log(`    Right edge: x=${o.x+o.width} > effectiveW=${effectiveW}`);
  }
});

// Simulate the algorithm's obstacle processing
let availableSpaces = [
